...
public class Query {
  ...
  /**
   * Used to represent Queries within the system. All fields are final to prevent modifications
   * after creation. Each instance have a UUID that can be used to identify it.
   *
  ...
  public static List<Query> createQuery(String body, List<Node> destinations) {
    return destinations.stream()
        .map(destination -> new Query(body, destination))
  ...
  public static List<Query> createQuery(List<String> bodies, List<Node> destinations) {
    ...
    return IntStream.range(0, bodies.size())
        .mapToObj(i -> new Query(bodies.get(i), destinations.get(i)))
        .collect(Collectors.toList());
  }
}
